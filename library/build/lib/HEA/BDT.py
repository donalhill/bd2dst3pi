...
def signal_background(data1, data2, column=None,range_column=None, grid=True,
    ...
    **kwds       : dict
        other plotting keyword arguments, to be passed to hist function
    ...
    for i, col in enumerate(data1.columns): # data.columns = the column labels of the DataFrame.
        ...
        h.set_label_hist(ax, latex_branch, unit, bin_width=bin_width, density=False, fontsize=20)
        pt.fix_plot(ax, factor_ymax=1+0.3, show_leg=True, fontsize_ticks=15., fontsize_leg=20.)
...
def correlations(data, fig_name=None,folder_name=None, title=None, **kwds):
    ...
    folder_name  : str
        name of the folder where to save the plot
    **kwds       : dict
        other plotting keyword arguments, to be passed to DataFrame.corr()
    ...
    pt.save_fig(fig, f"corr_matrix_{fig_name}", folder_name=f'BDT/{folder_name}')
...
def BDT(X_train, y_train, classifier='adaboost', **hyperparams):
    ...
    add_in_dic('learning_rate', hyperparams, 0.1) # Learning rate shrinks the contribution of each tree by alpha    
...
def plot_roc(X_test, y_test, bdt, BDT_name=None):
    ...
    bdt           : trained BDT
    BDT_name      : str, name of the BDT, used for the name of the saved plot
    folder_name   : str, name of the folder where to save the BDT
    ...
    pt.show_grid(ax)
    pt.fix_plot(ax, factor_ymax=1.1, show_leg=False, fontsize_ticks=20., ymin_to_0=False)
    ## Save the results -----
...
def apply_BDT(df_tot, df_train, bdt, BDT_name=None, save_BDT=False, kind_data='common'):
    """ 
    Apply the BDT to the real data in df_tot['data_strip']
    Add the BDT output as a new variable in df_tot['data_strip']
    Save df_tot['data_strip'] in a root file {loc['out']}/root/data_strip_{BDT_name}.root (branch 'DecayTreeTuple/DecayTree')
    In addition, save the BDT output in a separated root file {loc['out']}/tmp/BDT_{BDT_name}.root (branch 'BDT')
    Also save the BDT in a pickle file {loc['out']}/pickle/bdt_{BDT_name}.pickle
    ...
    # Apply the BDT to the dataframe that contains only the variables used in the training, in the right order
    ...
    df = pd.DataFrame()
    df['BDT'] = df_tot['BDT']
    #df.to_root(loc['out'] + f"tmp/BDT{BDT_name}.root",key = 'BDT')
