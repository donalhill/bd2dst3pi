...
Anthony Correia
08/02/21
Functions for the parameters of the fits
"""
...
def retrieve_params(file_name, folder_name=None):
    ...
    """
    return retrieve_json(file_name=file_name+'_params', folder_name=folder_name)
...
def get_params_without_BDT(df_params, retrieve_err=False):
    ...
    retrieve_err    : bool
        ...
        i.e., the variables that contain '_err'
    ...
    Notes
    -----
    With my notations, for a BDT cut `BDT > 0.2`, the variables are named with the `|BDT0.2` ending.
...
def show_latex_table(name, path=None):
    ...
    Notes
    -----
    the latex table must have been already generated, for instance with `json_to_latex_table()`. 
    """
    directory = create_directory(loc['tables'], path)
