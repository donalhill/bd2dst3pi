...
def assert_needed_variables(needed_variables, df):
    ...
    -------
    bool
        True if all the variables in `needed_variables` arein the dataframe `df`
...
def load_saved_root(name_data, vars=None, folder_name="", tree_name = 'DecayTree', cut_BDT=None, method='read_root'):
    """
...
def get_dataframe_from_raw_branches_functions(df, raw_branches_functions, mode='new', functions=definition_functions):
    ...
    for raw_branch_function in raw_branches_functions:
        ...
        if name_function is not None:
            new_branch = RVariable.get_branch_from_raw_branch_name_function(raw_branch, name_function)
