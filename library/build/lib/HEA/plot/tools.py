...
- Functions which, from the root branch name, gets the unit, name of the branch and particle (from branch.py)
...
def fix_plot(ax, factor_ymax=1.1, show_leg=True, fontsize_ticks=default_fontsize['ticks'], 
    ...
    axis            : 'y' or 'x'
        if 'x', `ymin_to_0` and `factor_ymax` acts on the x axis
    """
...
def get_latex_branches_units(branches):
    """ Get the latex branches and units assiated to branches
    Parameters
    ----------
...
def set_folder_name_from_data_name(kwargs, data_names):
