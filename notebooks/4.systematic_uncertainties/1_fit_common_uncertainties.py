...
def main():
    ...
    parser.add_argument('mode', type=int, help='mode (1) Kpipi, (2) pipipi or (3) Ds')
...
def launch (args):
    ...
    f_params_dict['3'] = ['mu_Ds', 'sigma_Ds', 'n_sig_Ds']
    ...
    for param in 'mu', 'sigma', 'n_sig':
        f_params[param + '_Ds'] = {}
        f_params[param + '_Ds']['val'] = Ds_params[param + f'_DstDs']
        f_params[param + '_Ds']['std'] = Ds_params[param + f'_DstDs' +'_err']
    ...
    for i in range(start, n_iters):
        ...
        r_params = {} # fixed parameters, changed or not (r for random)
        
        for param in f_params: # loop on the parameters that are possibly changed or not
            if param in f_params_dict[k]: # if in the chosen mode, the value of the parameters is supposed to change
        ...
        initial_values = {
            ...
            'lambda'   :{ 'value': pipipi_params['lambda']       , 'low': -1.                           , 'high': -0.0000000001        , 'floating': True},
