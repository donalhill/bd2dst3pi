...
def save_file(fig, name_file,name_folder=None, alt_name_file=None, directory=f"{loc.PLOTS}/"):
    ...
    fig.savefig(op.join(directory, f"{remove_space(remove_latex(name_file))}.pdf"), dpi=1200)
    fig.savefig(op.join(directory, f"{remove_space(remove_latex(name_file))}.png"), dpi=600)
...
#     name_particle = particle_names[particle]
    ...
    plot_hist(dfs, variable, name_variable, unit_var, **kwargs)
...
def plot_divide_particle(dfs, variable, **kwargs): 
    ...
    plot_divide(dfs, variable, name_variable, unit_var, **kwargs)
...
def plot_hist2d_particle(df, variables, **kwargs):
    ...
    plot_hist2d(df, variables, name_variables=name_variables, unit_variables=unit_vars, **kwargs)
